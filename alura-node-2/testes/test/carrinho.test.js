import Carrinho from '../src/carrinho.js';
import Item from '../src/item.js';

describe('Teste no carrinho', () => {
    const carrinho = new Carrinho();

    it('Deve iniciar vazio', () => {
        expect(carrinho.subtotal).toBeNull();
    });

    it('Deve ter itens', () => {
        const item1 = new Item('Banana', 2, 5);
        const item2 = new Item('Maça', 0.5, 1);

        carrinho.adiciona(item1);
        carrinho.adiciona(item2);

        expect(typeof carrinho).toBe('object');
        expect(carrinho.itens[0]).toBe(item1);
        expect(carrinho.itens[1]).toBe(item2);

        expect(carrinho.itens).toContain(item1);
        expect(carrinho.itens).toContain(item2);
    });

    it('Deve ter a propriedade "Total" na inicialização', () => {
        const carrinho2 = new Carrinho();

        expect(carrinho2).toHaveProperty('total');
    });

    it('Deve lançar erro ao tentar finalizar compra com carrinho vazio', () => {
        function englobaErroCarrinho() {
            const carrinho2 = new Carrinho();
            carrinho2.finalizaCompra();
        }

        expect(englobaErroCarrinho).toThrowError('Carrinho de compras vazio');
    });

    it('Deve adicionar o frete', () => {
        carrinho.adicionaFrete(10);

        expect(carrinho.frete).toBe(10);
    });

    it('Deve retonar valor total do carrinho', () => {
        const total = carrinho.calculaTotal();

        expect(total).toBe(20.5);
    });

    it('Deve finalizar compra', () => {
        const { subtotal, frete, total } = carrinho.finalizaCompra();

        expect(subtotal).toStrictEqual(10.5);
        expect(frete).toStrictEqual(10);
        expect(total).toStrictEqual(20.5);
    });
});
