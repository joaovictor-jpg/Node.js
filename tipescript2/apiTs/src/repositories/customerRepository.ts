...
async function addCustomer(customer: Customer): Promise<Customer> {
    return new Promise((resolve, reject) => {
        if (!customer.nome || !customer.cpf)
            return reject(new Error(`Invalido custormer.`));
        const newCustomer = new Customer(customer.nome, customer.cpf);
...
async function updateCustomer(id: number, customerData: Customer): Promise<Customer> {
    return new Promise((resolve, reject) => {
        ...
        if (customerData.nome && customers[index].nome !== customerData.nome)
            customers[index].nome = customerData.nome;
